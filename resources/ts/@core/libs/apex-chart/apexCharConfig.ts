    ...
    tooltip: {
      ...
      custom(data: any) {
        return `<div class='bar-chart pa-2'>
          <span>₱${data.series[data.seriesIndex][data.dataPointIndex]}.00</span>
        </div>`
      },
    },
    yaxis: {
...
export const getDonutChartConfig = (themeColors: ThemeInstance['themes']['value']['colors']) => {
  ...
  return {
    stroke: { width: 0 },
    labels: ['Journal Request', 'Payment Request', 'Active Clients', 'Published Journals'],
        ...
        donut: {
          labels: {
            ...
            total: {
              ...
              label:'Operations',
              formatter(w: { globals: { seriesTotals: any[]; series: string | any[] } }) {
                const totalValue = w.globals.seriesTotals.reduce((a: number, b: number) => {
                  return a + b;
                }, 0);
                return totalValue;
              },
