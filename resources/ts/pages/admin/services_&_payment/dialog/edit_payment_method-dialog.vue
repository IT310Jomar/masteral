...
const edit_payment = () => {
  ...
  axios.post(environment.API_URL + `auth/admin/edit-payment-method/${props.row.id}`, formData, {
      headers: {
...
onMounted(() => {
  if (props.row != null) {
    paymentname.value = props.row?.name;
    account.value = props.row?.account;
    if (props.row.qr_code_image != null) {
      qrcode.value = 'Yes';
      qr_code.value = props.row.qr_code_image;
    } else {
      qrcode.value = 'No';
      qr_code.value = null; // Set qr_code.value to null if no QR code image
    }
  }
  initializeData();
});
...
<template>
  <section>
    <v-row>
      <v-col>
        <VCard
          max-height="550px"  class="d-flex flex-column flex-grow-1 overflow-auto"
          title="Edit Payment Method"
        >
          <VDivider />
          <VCardText>
            <VForm ref="refsubmit" @submit.prevent="onSubmit">
              <VCardText style="margin-top: -4%">
                <VRow>
                  <VCol class="mb-2" xs="12" md="6">
                    <VTextField
                      v-model="paymentname"
                      type="text"
          ...
          <VCol>
            ...
            ></VTextField>
                ...
                <VRow v-if="qrcode === 'Yes'">
                  <VCol class="mb-2" xs="12">
                    <v-file-input
                      accept=".jpg,.jpeg,.png"
                      ref="fileInputImage"
                ...
                <VRow style="padding-left: 22%" v-if="qrcode === 'Yes'">
                  <VCol xs="12">
                    <div
    ...
    <VSnackbar v-model="isSnackbarSuccess" :timeout="4000" color="success">
      Success! Payment Method Data Edited.
    </VSnackbar>
