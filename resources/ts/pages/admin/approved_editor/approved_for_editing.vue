...
// import printDocument from "./print/assignedPrintDialog.vue";
...
//Pending Pagination
const paginatedEditingData = computed(() => {
  const startIndex = (currentPage.value - 1) * itemsPerPage.value;
  const endIndex = startIndex + itemsPerPage.value;
  return EditingData.value.slice(startIndex, endIndex);
});

let totalPages = computed(() => {
  if (EditingData.value.length > 0) {
    return Math.ceil(EditingData.value.length / itemsPerPage.value);
  } else {
    return 0;
  }
});
...
const searchApproved = () => {
  axios
    .get(environment.API_URL + "auth/admin/search-editing-journal", {
      params: {
    ...
    .then((response) => {
      if (response.data.success) {
        EditingData.value = response.data.searchEditingData;
      }
    })
    .catch((error) => {
      console.log("Error");
...
const getAssignedList = () => {
  axios
    .get(environment.API_URL + "auth/admin/get-editing-journal",{
      params: {
        token: token
      }
    })
    .then((response) => {
      if (response.data.success) {
        EditingData.value = response.data.editingdata;
...
async function generatePdf(tableRef: any, logoUrl: string | HTMLImageElement | HTMLCanvasElement | Uint8Array | RGBAData) {
  try {
    ...
    const title = "DIT.ADS- Assigned List Management Report";
...
<template>
  <section>
    <VCard title="Approved Editing Journal List">
      <!-- <VDivider/> -->
      <!-- <VCardText>
        <VRow>
      ...
      <VTable ref="tableRef">
        ...
        <thead>
          <tr>
            ...
            <th class="text-center">Assigned Editor</th>
            <th class="text-center">Client Fullname</th>
            <th class="text-center">Date of Request</th>
            <th class="text-center">Client Uploaded File</th>
            <th class="text-center">Editor Status</th>
            <th class="text-center">Date of Editing</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(row, index) in paginatedEditingData" :key="row.id">
            ...
            <!-- <td class="text-center">
              <VBtn icon size="x-small" variant="text" class="icon-container">
                ...
                <VMenu activator="parent">
                  <VList>
                    <VListItem :to="'/editor/assigned-view/' + row.id">
                      <template #default>
        ...
        <!-- 👉 table footer  -->
        <tfoot v-if="EditingData?.length == 0">
          <tr>
            <td colspan="8">
    ...
    >
      <printDocument :Data="paginatedEditingData" />
    </VDialog> -->
  </section>
</template>
<style scoped>
