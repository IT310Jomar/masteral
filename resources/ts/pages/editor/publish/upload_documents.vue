...
//Pending Pagination
const paginatedEditedFile = computed(() => {
  const startIndex = (currentPage.value - 1) * itemsPerPage.value;
  const endIndex = startIndex + itemsPerPage.value;
  return EditedFile.value.slice(startIndex, endIndex);
});

let totalPages = computed(() => {
  if (EditedFile.value.length > 0) {
    return Math.ceil(EditedFile.value.length / itemsPerPage.value);
  } else {
    return 0;
  }
});
...
//         EditedFile.value = response.data.EditedFile;
//       }
//     })
//  }
// }
...
const searchApproved = () => {
  axios
    .get(environment.API_URL + "editor/editor/search-edited-journal/" + editor_id[0].id, {
      params: {
    ...
    .then((response) => {
      if (response.data.success) {
        EditedFile.value = response.data.searchEdited;
      }
    })
    .catch((error) => {
      console.log("Error");
...
const getEditedList = () => {
  axios
    .get(environment.API_URL + "editor/editor/get-edited-uploaded-file/" + editor_id[0].id,{
      params: {
        token: token
      }
    })
    .then((response) => {
      if (response.data.success) {
        EditedFile.value = response.data.editedData;
...
async function generatePdf(tableRef: any, logoUrl: string | HTMLImageElement | HTMLCanvasElement | Uint8Array | RGBAData) {
  try {
    ...
    const title = "DIT.ADS- Upload Edited List Management Report";
...
<template>
  <section>
    <VCard title="Upload Journal List">
      <!-- <VDivider/> -->
      <!-- <VCardText>
        <VRow>
      ...
      <VTable ref="tableRef">
        ...
        <thead>
          <tr>
            ...
            <th class="text-center">Edited Uploaded File</th>
            <th class="text-center">Action</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(row, index) in paginatedEditedFile" :key="row.id">
            ...
            <td class="text-center">
              <VBtn icon size="x-small" variant="text" class="icon-container">
                ...
                <VMenu activator="parent">
                  <VList>
                    <VListItem :to="'/editor/upload-published-view/' + row.id">
                      <template #default>
        ...
        <!-- 👉 table footer  -->
        <tfoot v-if="EditedFile?.length == 0">
          <tr>
            <td colspan="8">
    ...
    >
      <printDocument :Data="paginatedEditedFile" />
    </VDialog> -->
