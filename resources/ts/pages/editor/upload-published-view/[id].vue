...
import editedFile from "@/pages/editor/upload-published-view/editedUpload_dialog/reupload.vue";
...
const editedUpload = ref(false);
const requestID = ref();

const editedJournal = () => {
  editedUpload.value = true
    ...
    .then((response) => {
      if (response.data.success) {
        ...
        if(RequestData.value && 'editor_uploaded_file' in RequestData.value){
          ...
          if(RequestData.value.edited_upload === 0) {
            router.push("/editor/published_journals");
          }
        }
...
<template>
  <section>
    <VCard title="Upload Published Journal">
      <v-btn
        link
      ...
      <v-card-text v-else-if="RequestData && RequestData.editor_status === 'Approved' && RequestData.release_status === 1 && RequestData.editor_id === editor_id[0].id">
        <VRow>
          <VCol cols="8">
            <VCard>
              <div style="height: 80vh" v-if="pdfUrl === '/storage/published_journals/null'">
                <VImg
                  :src="logo"
                  style="
              ...
              <VToolbar>
                <VRow>
                  <VCol class="text-center">
                    <VBtn color="red" variant="outlined" @click="editedJournal">
                      <VIcon start size="25" icon="tabler:pencil-down" />
                        Upload Edited Journal
                    </VBtn>
                  </VCol>
                </VRow>
              </VToolbar>
            </VForm>
            <VCard class="mt-3" title="Edited By:" v-if="RequestData.request_status == 'Approved'">
              <VDivider />
              <VCardText>
                <VRow>
                  <VCol>
    ...
    <VDialog v-model="editedUpload" scrollable max-width="450">
      <editedFile
        :row="requestID"
        @close="editedUpload = false"
