...
const paginatedDocumentData = computed(() => {
  ...
  return DocumentData.value.slice(startIndex, endIndex);
});

let totalPages = computed(() => {
  if (DocumentData.value.length > 0) {
    return Math.ceil(DocumentData.value.length / itemsPerPage.value);
  } else {
    return 0;
  }
});
  ...
  axios.get(environment.API_URL + 'auth/admin/paid-list/range', {
    ...
    .then((response) => {
      if (response.data.success) {
        ...
        DocumentData.value = response.data.DocumentData;
      }
    })
 }
}
...
const searchApproved = () => {
  axios
    .get(environment.API_URL + "editor/editor/search-assigned-journal/" + editor_id[0].id, {
      params: {
    ...
    .then((response) => {
      if (response.data.success) {
        DocumentData.value = response.data.searchAssigned;
      }
    })
    .catch((error) => {
      console.log("Error");
...
const getAssignedList = () => {
  axios
    .get(environment.API_URL + "editor/editor/get-assigned-editor/" + editor_id[0]?.id,{
      params: {
        token: token
      }
    })
    .then((response) => {
      if (response.data.success) {
        DocumentData.value = response.data.approveddata;
...
<template>
  <section>
    <VCard title="Assigned Document List">
      <!-- <VDivider/> -->
      <!-- <VCardText>
        <VRow>
      ...
      <VTable ref="tableRef">
        ...
        <tbody>
          <tr v-for="(row, index) in paginatedDocumentData" :key="row.id">
            ...
            <td class="text-center">
              ...
              {{ moment(row.date_time_requests).format("hh:mm A") }}
            </td>
            <td class="text-center">
              {{ row.uploaded_file }}
            </td>
        ...
        <!-- 👉 table footer  -->
        <tfoot v-if="DocumentData?.length == 0">
          <tr>
            <td colspan="8">
