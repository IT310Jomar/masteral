...
if (dateRange.value != null) {
  ...
  axios.get(environment.API_URL + 'client/client/published-journal-list/range/' + client_id[0].id, {
    params: {
    ...
    .then((response) => {
      if (response.data.success) {
        ...
        PublishedJournal.value = response.data.publishedData;
      }
    })
 }
}
...
const searchPublished = () => {
  axios
    .get(environment.API_URL + "client/client/search-published-journal/" + client_id[0].id, {
      params: {
...
const getPublishedList = () => {
  axios
    .get(environment.API_URL + "client/client/get-published-journal/" + client_id[0].id, {
      params: {
        token: token
      }
    })
  ...
  <section>
    <VCard title="Published Journal List">
      ...
      <VCardText>
        <VRow>
          <VCol cols="6" offset="2" style="display:flex; align-items:center; margin-left: 25%;">
            ...
            <VBtn color="yellow" @click.prevent="searchRange" class="ml-2">Search</VBtn>
          </VCol>
        </VRow>
        <hr class="mt-5" />
      </VCardText>
      ...
      <VTable ref="tableRef">
        ...
        <tbody>
          <tr v-for="(row, index) in paginatedPublishedJournal" :key="row.id">
            ...
            <td class="text-center">
              <VBtn icon size="x-small" variant="text" class="icon-container">
                ...
                <VMenu activator="parent">
                  <VList>
                    <VListItem :to="'/client/publish_journal/download-view/' + row.id">
                      <template #default>
    ...
    >
      <printDocument :Data="paginatedPublishedJournal" />
    </VDialog> -->
  </section>
</template>
<style scoped>
